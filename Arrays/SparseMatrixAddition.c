...
int addSparse(struct Sparse *A, int na, struct Sparse *B, int nb, struct Sparse *C){
    int i=0, j=0, k=0;
...
int main(){
    ...
    struct Sparse *AddS = (struct Sparse*)malloc((na + nb) * sizeof(struct Sparse));
    ...
    printf("\nAddition:");
    int nc = addSparse(SA, na, SB, nb, AddS); 
    printSparse(AddS, nc);
